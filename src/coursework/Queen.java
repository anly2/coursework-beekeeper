...
class Queen extends Bee
{
	 ...
	 * 
	 * @return Returns the Queen itself.
	 * @throws BeeDeath if the Queen died
	 */
	public Queen anotherDay () throws BeeDeath
	{
		...
		if (age >= maxAge) //Is old
			if (Rand.chance((int) Config.get("chance death bee")))
				throw new BeeDeath (this, "old age");
		...
		{				
			try {
				...
				hive.garden.getInterface().event("hiveOverflow", hive, e, "There were no available cells for the Queen to lay an Egg in!");
			}
		}
	 ...
	 * 		<b>-fixed-</b> Thus the {@link Rand Random Number Generator} is offset by <b>+1</b> from the actual seed. <br />
	 * 		<s>The effect is small but may actually lead to a slightly different simulation than the one saved.</s>
	 * </p>
	 * 
