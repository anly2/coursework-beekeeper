...
public class Garden {
	 ...
	 * @return Returns the Garden itself or null if there are no hives in this Garden
	 * @see Flower#anotherDay()
	 * @see Hive#anotherDay()
	 */
	public Garden anotherDay () {
		...
		if (hives.size() <= 0)
			return null; //consider the garden dead as well
		
		//Have the flowers grow
		for (int i = 0; i < flowers.size(); i++)
			if (flowers.get(i) == null)
				flowers.remove(i--);
			else
